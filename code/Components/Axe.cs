        ...
        if ( target.Components.TryGet<ItemComponent>( out var item ) )
        {
            if ( item.Name == "Wooden Log" )
            {
                if ( PrefabLibrary.TryGetByPath( "prefabs/items/split_wooden_log.prefab", out var log ) )
                {
                    ...
                    var left = SceneUtility.GetPrefabScene( log.Prefab ).Clone( target.WorldPosition, target.WorldRotation );
                    var right = SceneUtility.GetPrefabScene( log.Prefab ).Clone( target.WorldPosition, target.WorldRotation.RotateAroundAxis( Vector3.Forward, 180f ) );

                    if ( left.Components.TryGet<Rigidbody>( out var leftBody ) )
                        leftBody.Velocity = left.WorldRotation * Vector3.Right * 100f;
                    if ( right.Components.TryGet<Rigidbody>( out var rightBody ) )
                        rightBody.Velocity = right.WorldRotation * Vector3.Right * 100f;

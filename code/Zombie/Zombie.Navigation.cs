...
public partial class Npc
{
	...
	float[] getDanger()
	{
		...
		foreach ( var direction in possibleDirections )
		{
			...
			// Perform ray trace to check for collisions with walls
			var hitResult = Scene.Trace.Ray( startPosition, endPosition )
				.IgnoreGameObjectHierarchy( GameObject )
				.WithoutTags( "NPC" ) // Adjust tags as necessary
				.Run();

			directionDanger[currentDirection] = hitResult.Hit ? 5f : 0f;

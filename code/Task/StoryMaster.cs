...
using static GeneralGame.TaskMaster;

namespace GeneralGame;

public class GeneralScriptedEvent
{
	/// <summary>
	...
	public GeneralScriptedEvent() { }
}

public class GeneralDay
{
	[Property]
	public List<GeneralScriptedEvent> ScriptedEvents { get; set; }
...
{
	public class GeneralStoryProgression
	{
		[JsonInclude]
		...
		public GeneralStoryProgression() { }
	}
	...
	/// </summary>
	[Property]
	public Dictionary<int, GeneralDay> StoryDays { get; set; } = new();
	...
	[Property]
	[HostSync]
	public GeneralStoryProgression StoryProgression { get; set; }
	...
	public GeneralDay NextGeneralDay => StoryDays.TryGetValue( StoryProgression.StoryDay + 1, out var generalDay ) ? generalDay : null;
	...
	{
		if ( CurrentGeneralDay == null ) return;

		foreach ( var scriptedEvent in CurrentGeneralDay.ScriptedEvents )
		{
	...
	public void LoadStoryProgression()
	{
		if ( FileSystem.OrganizationData.FileExists( "story.json" ) )
			StoryProgression = FileSystem.OrganizationData.ReadJsonOrDefault<GeneralStoryProgression>( "story.json" );
		else
		{
	...
	public void LoadEventPool()
	{
		...
		if ( CurrentGeneralDay.RandomEvents.ContainsKey( EventRarity.Common ) )
		{
			int eventsPicked = 0;

			for ( int common = 0; common < CurrentGeneralDay.RandomEvents[EventRarity.Common]; common++ )
			{
		...
		if ( CurrentGeneralDay.RandomEvents.ContainsKey( EventRarity.Uncommon ) )
		{
			int eventsPicked = 0;

			for ( int uncommon = 0; uncommon < CurrentGeneralDay.RandomEvents[EventRarity.Uncommon]; uncommon++ )
			{
		...
		if ( CurrentGeneralDay.RandomEvents.ContainsKey( EventRarity.Rare ) )
		{
			int eventsPicked = 0;

			for ( int rare = 0; rare < CurrentGeneralDay.RandomEvents[EventRarity.Rare]; rare++ )
			{
	...
	public void RandomizeClothing()
	{
		...
		var allClothingShops = Scene.GetAllComponents<ItemComponent>()
	...
	{
		foreach ( var @event in CurrentGeneralDay.ScriptedEvents )
		{
	...
	protected override void OnFixedUpdate()
	{
...

		{
			...
			if ( !CurrentGeneralDay.Completed )
				if ( CurrentGeneralDay.ScriptedEvents.All( x => x.Completed || x.Triggered && !x.CompletionNecessary ) )
				{
					CurrentGeneralDay.Completed = true;
					Log.Info( "All story scripted event requirements completed, story can now progress" );
				}
		}
	}

	internal async void BeginScriptedEvent( GeneralScriptedEvent scriptedEvent, float delay = 0f )
	{

...
VS
{
	...
	float g_flWarpIntensityScale < UiGroup( "Surface Warp,3/,0/0" ); Default1( 1 ); Range1( 0, 20 ); >;
	float g_flWarpPatternScale < UiGroup( "Surface Warp,3/,0/0" ); Default1( 0.5 ); Range1( 0, 10 ); >;
	float2 g_vGeometryWarpScrollSpeed < UiGroup( "Surface Warp,3/,0/0" ); Default2( 0.5,0.25 ); >;
	...
	PixelInput MainVs( VertexInput v )
	{
		...
		float l_0 = g_flWarpIntensityScale;
		float2 l_1 = i.vTextureCoords.xy * float2( 1, 1 );
		float l_2 = g_flWarpPatternScale;
		float2 l_3 = l_1 * float2( l_2, l_2 );
		float2 l_4 = g_vGeometryWarpScrollSpeed;
		float2 l_5 = l_4 * float2( g_flTime, g_flTime );
		float2 l_6 = TileAndOffsetUv( l_3, float2( 1, 1 ), l_5 );
		float l_7 = Simplex2D( l_6 );
		float l_8 = l_0 * l_7;
		float3 l_9 = i.vNormalWs * float3( l_8, l_8, l_8 );
...
PS
{
	...
	float g_flSelfillumStrenght < UiGroup( "Self Illum,1/,0/0" ); Default1( 0 ); Range1( 0, 1 ); >;
	float g_flRoughnessAmount < UiGroup( "PBR,8/,0/0" ); Default1( 0 ); Range1( 0, 1 ); >;
	float g_flMetalnessAmount < UiGroup( "PBR,8/,0/0" ); Default1( 0 ); Range1( 0, 1 ); >;
	float g_flAmbientAmount < UiGroup( "PBR,8/,0/0" ); Default1( 0 ); Range1( 0, 1 ); >;
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		float2 l_12 = saturate( lerp( l_11, l_4, 1 ) );
		...
		float4 l_21 = Tex2DS( g_tSelfIllum, g_sSampler0, l_4 );
		float l_22 = g_flSelfillumStrenght;
		float4 l_23 = lerp( float4( 0, 0, 0, 0 ), l_21, l_22 );
		float4 l_24 = Tex2DS( g_tAlphaMask, g_sSampler0, l_4 );
		float l_25 = g_flRoughnessAmount;
		float l_26 = g_flMetalnessAmount;
		float l_27 = g_flAmbientAmount;
